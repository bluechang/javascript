...
;(function($, window){
	...
	function Slidebar(elem, options){
		...
		//父容器宽度,即展开和收起的宽度
		t.parentWidth = t.opts.hasPadding ? t.$parent.outerWidth() : t.$parent.width(); 
		//是否显示内容	
	...
	Slidebar.prototype.initStyle = function(){
		...
		if( t.isShow ){
			t.$parent.css( t.direction, 0 );
			t.open();
		}else{
			t.$parent.css( t.direction, -t.parentWidth );
			t.close();
		}
	};
	...
	Slidebar.prototype.getDirection = function(){  
		...
		var offsetLeft = t.$parent.offset().left;

		if( offsetLeft >= t.parentWidth ) 		//判断左偏移是否大于父容器宽度
			return 'right';
		else
			return 'left';
	};
	...
	Slidebar.prototype.close = function(){
		...
		t.$parent.stop(true).animate( 
			t.direction == 'right' ? { right: -t.parentWidth } : { left: -t.parentWidth }, 
			t.opts.speed 
		);
	};
